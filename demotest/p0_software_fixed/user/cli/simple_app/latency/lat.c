...
static int prefill(struct queue_ctx *qctx)
{
	...
	for (i = 0; i < qdepth; i++) {
		...
		req_buf[i]->flags |= IFC_QDMA_SOF_MASK;
		req_buf[i]->flags |= IFC_QDMA_EOF_MASK;
	}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < global_flags->packets ; i++) {
	...
	while (1) {
		...
		clock_gettime(CLOCK_MONOTONIC, &qctx->end_time);
		time_diff(&(qctx->end_time), &(qctx->start_time), &result);
		printf("Completions %u i:%u sec:%lu nsec:%lu\n", q->consumed_head,
				i, result.tv_sec, result.tv_nsec/1);
			break;
	}
